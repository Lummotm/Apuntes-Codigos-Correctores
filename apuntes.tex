...
\subsection{4.3 Decodificación Única y el Sistema de Welch-Berlekamp}

Dado un vector recibido $r = (r_1, \dots, r_n)$, el objetivo en decodificación única (capacidad correctora hasta $t = \lfloor \frac{n-k}{2} \rfloor$ errores) es encontrar un polinomio bivariado de la forma $Q(x,y) = Q_0(x) + Q_1(x)y \in \mathbb{F}_q[x,y]$.

Establecemos las restricciones de grado basándonos en los parámetros: $l_0 = n - 1 - t$ y $l_1 = n - 1 - t - (k - 1)$. Para cada punto de evaluación $x_i$ y valor recibido $r_i$, planteamos el sistema lineal homogéneo:
\[ Q_0(x_i) + r_i Q_1(x_i) = 0, \quad \text{para } i = 1, \dots, n \]

Definiendo los polinomios mediante sus coeficientes incógnitos:
\[ Q_0(x) = \sum_{j=0}^{l_0} Q_{0,j} x^j \quad \text{y} \quad Q_1(x) = \sum_{j=0}^{l_1} Q_{1,j} x^j \]

\textbf{Construcción de las Matrices por Bloques $A$ y $B$:}\\
El sistema de $n$ ecuaciones se estructura mediante la matriz de bloques $M = (A \mid B)$:
\begin{itemize}
    \item \textbf{Matriz $A$ (Asociada a $Q_0$):} Matriz de tipo $n \times (l_0 + 1)$ con las potencias de los puntos de evaluación.
    \[ A = \begin{pmatrix} 
    ...
    \item \textbf{Matriz $B$ (Asociada a $Q_1$):} Matriz de tipo $n \times (l_1 + 1)$ donde cada fila de potencias se multiplica por el correspondiente valor recibido $r_i$.
    \[ B = \begin{pmatrix} 
...
El sistema completo se expresa como el producto de la matriz de bloques por el vector de coeficientes:
\[ (A \mid B) \cdot \begin{pmatrix} Q_{0,0} \\ \vdots \\ Q_{0,l_0} \\ Q_{1,0} \\ \vdots \\ Q_{1,l_1} \end{pmatrix} = \mathbf{0} \]

Cualquier vector no nulo perteneciente al \textit{núcleo por la derecha} (right kernel) de la matriz de bloques proporciona una solución válida para el polinomio $Q(x,y)$. El polinomio original de información se recupera directamente mediante la división analítica:
\[ f(x) = -\frac{Q_0(x)}{Q_1(x)} \]

\textbf{Teo. 4.3.1:} Si han ocurrido menos de $\frac{d}{2}$ errores, entonces existe al menos un polinomio no nulo $Q(x,y)$ que satisface las condiciones del sistema.

\textbf{Teo. 4.3.2:} Si la palabra transmitida es generada por $f(x)$ y el número de errores es menor a $\frac{d}{2}$, entonces se cumple rigurosamente que $f(x) = -\frac{Q_{0}(x)}{Q_{1}(x)}$.

\textbf{Algoritmo 4.3.3 (Histórico de Peterson):}\\
Este método clásico divide el problema de decodificación única en dos fases algebraicas secuenciales:
\begin{enumerate}
\item Se calculan los síndromes iniciales $S_{i} = r(\beta^{i})$ para $i=1,2,...,n-k$.
\item Se encuentra la solución $Q_{1}(x)$ de menor grado resolviendo un sistema de ecuaciones matricial formado exclusivamente por estos síndromes.
\item Se calculan analíticamente los ceros de $Q_{1}(x)$, que llamaremos $\beta^{i_{1}},...,\beta^{i_{t}}$.
\item Se hallan los valores de los errores resolviendo un sistema de ecuaciones lineales final o utilizando una fórmula directa. La salida es el vector de error corregido $(e_{1},e_{2},...,e_{n})$.
\end{enumerate}
...
Estructura de ideales en anillos cocientes, raíces de la unidad, polinomios ciclotómicos, clases cíclicas (cosets), polinomios generadores, elementos idempotentes y descodificación porMeggit.\\
Bibliografía:
\begin{itemize}
...
\subsection{8.2 Factorización de Xn-1 y Conjuntos Ciclotómicos}

Puesto que todo ideal en un anillo de principales es principal, cada código cíclico está unívocamente determinado por un único polinomio mónico de grado mínimo, denominado \textbf{polinomio generador} $g(x)$, el cual debe ser un divisor exacto de $x^n - 1$ en $\mathbb{F}_q[x]$. La dimensión del código resultante viene dada por $k = n - \operatorname{deg}(g(x))$.

Para hallar los factores irreducibles de $x^n-1$ sobre el cuerpo base $\mathbb{F}_q$, extendemos el análisis a su cuerpo de escisión $\mathbb{F}_{q^m}$ (donde $n | q^m-1$). Sea $\beta$ una raíz primitiva $n$-ésima de la unidad. Las raíces del polinomio se agrupan en órbitas cerradas bajo la acción del automorfismo de Frobenius. Estas órbitas se denominan \textbf{clases o cosets ciclotómicos} módulo $n$ respecto a $q$:
\[
C_j = \left\{ j, \ j\cdot q, \ j\cdot q^2, \ \dots, \ j\cdot q^{s-1} \pmod n \right\}
\]
donde $s$ es el menor entero positivo tal que $j\cdot q^s \equiv j \pmod n$. Cada clase ciclotómica define de forma exacta un polinomio mínimo irreducible sobre el cuerpo base $\mathbb{F}_q$:
\[
m_j(X) = \prod_{i \in C_j} (X - \beta^i)
\]
La factorización completa del espacio se obtiene seleccionando un representante minimal por cada clase diferenciada: $X^n - 1 = \prod_{j} m_j(X)$. Construir un código cíclico se reduce a elegir qué subconjunto de estos polinomios mínimos compondrán el polinomio generador $g(x)$.

\subsection{8.3 Elementos Idempotentes}
...
Su cálculo se realiza a partir del polinomio generador $g(x)$ y su polinomio de control asociado $h(x) = (x^n-1)/g(x)$. Como $\operatorname{mcd}(g(x), h(x)) = 1$, el algoritmo de Bézout asegura la existencia de dos polinomios $a(x), b(x)$ tales que:
\[
a(x)g(x) + b(x)h(x) = 1 \implies e(x) \equiv a(x)g(x) \pmod{x^n-1}
\]

\subsection{8.4 Descodificación Cíclica: Algoritmo de Atrapamiento de Errores (Meggitt)}
...
\newpage
\hlinecolor{}
\section{Tema 9: Códigos BCH}
Definición de códigos BCH, diseño por raíces consecutivas, construcción de la matriz de control en el cuerpo de extensión, cota fundamental y algoritmos de descodificación PGZ y Berlekamp-Massey.\\
Bibliografía:
\begin{itemize}
...
\subsection{9.1 Definición y Matriz de Control de Vandermonde}

Los códigos Bose-Chaudhuri-Hocquenghem (BCH) son una familia de códigos cíclicos diseñados específicamente para garantizar de forma analítica una capacidad correctora predeterminada, forzando la aparición de raíces consecutivas en el polinomio generador.

Elegimos una \textbf{distancia diseñada} $\delta$. Sea $\alpha$ un elemento de orden $n$ dentro de la extensión de cuerpos $\mathbb{F}_{q^m}$. Un código BCH de longitud $n$ sobre el cuerpo base $\mathbb{F}_q$ se define como el código cíclico cuyo polinomio generador $g(x)$ tiene entre sus raíces a $\delta-1$ potencias consecutivas de $\alpha$. 

En la práctica, $g(x)$ se construye como el mínimo común múltiplo de los polinomios mínimos asociados a las \textbf{clases ciclotómicas} de dichas potencias:
\[
g(x) = \operatorname{mcm}\left\{ m_b(x), m_{b+1}(x), \dots, m_{b+\delta-2}(x) \right\}
\]
donde $m_i(x) = Irr(\alpha^i, \mathbb{F}_q)$. Dado que las raíces conjugadas comparten el mismo polinomio mínimo, calcular las clases ciclotómicas reduce drásticamente el grado final de $g(x)$ y elimina redundancias.

Si el parámetro inicial es $b=1$, el código se clasifica como BCH en \textit{sentido estricto}. Si la longitud es máxima ($n = q^m-1$, lo que implica que $\alpha$ es un elemento primitivo de la extensión), el código es un BCH \textit{primitivo}.

\subsection{9.2 Descodificación Algebraica de Códigos BCH}

Sea $r(x) = c(x) + e(x)$ el polinomio recibido. Supongamos que han ocurrido $\nu \le t$ errores (donde $t = \lfloor \frac{\delta-1}{2} \rfloor$) en las posiciones desconocidas $j_1, j_2, \dots, j_\nu$. El polinomio de error se expresa como $e(x) = \sum_{l=1}^{\nu} e_{j_l} x^{j_l}$. El proceso de descodificación consta de cuatro etapas consecutivas:

\subsubsection{1. Cálculo de los Síndromes de Newton}
Se evalúa el polinomio recibido en las raíces de la definición del código. Esto genera $\delta-1 = 2t$ valores escalares pertenecientes al cuerpo de extensión $\mathbb{F}_{q^m}$:
\[
S_i = r(\alpha^{b+i-1}) = e(\alpha^{b+i-1}) = \sum_{l=1}^{\nu} e_{j_l} (\alpha^{j_l})^{b+i-1}, \quad \text{para } i = 1, 2, \dots, 2t
\]
Si todos los síndromes calculados resultan ser cero ($S_i = 0$), el algoritmo se detiene determinando que no se han producido errores.

\subsubsection{2. Determinación de los Polinomios y la Ecuación Clave}
Definimos las variables de localización como $X_l = \alpha^{j_l}$. Se construye el \textbf{polinomio localizador de errores} $\Lambda(x)$, cuyas raíces son los inversos de las posiciones erróneas, y el \textbf{polinomio evaluador de errores} $E(x)$ (o $\Omega(x)$):
\[
\Lambda(x) = \prod_{l=1}^{\nu} (1 - X_l x) = 1 + \Lambda_1 x + \dots + \Lambda_\nu x^\nu
\]
Si definimos el polinomio de síndromes como $S(x) = \sum_{i=1}^{2t} S_i x^{i-1}$, la relación fundamental que vincula los síndromes conocidos con las localizaciones e intensidades de los errores se conoce como la \textbf{Ecuación Clave de la Descodificación}:
\[
S(x)\Lambda(x) \equiv E(x) \pmod{x^{2t}}
\]
Sabiendo que el grado del polinomio evaluador cumple $\operatorname{deg}(E(x)) < \operatorname{deg}(\Lambda(x)) \le t$, este problema de congruencia polinómica se puede resolver de las siguientes maneras:
\begin{itemize}
    \item \textbf{Algoritmo de Peterson-Gorenstein-Zierler (PGZ):} Resuelve la Ecuación Clave transformándola en un sistema lineal mediante la inversión directa de la matriz de síndromes. Su complejidad computacional es alta ($O(t^3)$), por lo que solo es viable en diseños donde $t$ sea muy pequeño.
    \item \textbf{Algoritmo de Berlekamp-Massey:} Es un método iterativo altamente eficiente. Modela la Ecuación Clave como la síntesis del polinomio de conexión de longitud mínima para un registro LFSR que genere la secuencia de síndromes. Su complejidad óptima es de $O(t^2)$.
    \item \textbf{Algoritmo de Euclides Extendido:} Aplica el algoritmo de Euclides a los polinomios $x^{2t}$ y $S(x)$ hasta que el resto alcance un grado estrictamente menor que $t$.
\end{itemize}

\subsubsection{3. Búsqueda de Chien (Fase de Localización)}
Una vez obtenidos los coeficientes del polinomio $\Lambda(x)$, se realiza una evaluación exhaustiva del polinomio en todos los inversos de los elementos del cuerpo de extensión: $\Lambda(\alpha^{-l})$ para $l = 0, 1, \dots, n-1$. Si se verifica que:
\[
\Lambda(\alpha^{-l}) = 0
\]
se determina de forma unívoca que ha ocurrido un error en la posición posicional exacta $l$ del vector.

\subsubsection{4. Cálculo de Magnitudes de Error (Fórmula de Forney)}
\begin{itemize}
    \item \textbf{Códigos BCH Binarios ($q=2$):} Dado que el único valor de error no nulo posible en un bit es el $1$, la magnitud es trivial. Una vez localizada la posición mediante Chien, se corrige aplicando el operador NOT (volteando el bit).
    \item \textbf{Códigos BCH No Binarios ($q>2$):} Es necesario calcular el valor escalar del error. Se define el polinomio evaluador de errores como $\Omega(x) = S(x)\cdot\Lambda(x) \pmod{x^{2t}}$ (donde $S(x) = \sum S_i x^{i-1}$) y se aplica la \textbf{fórmula de Forney} para extraer la magnitud del error en la posición localizada:
    \[
    e_{j_l} = - \frac{\Omega(\alpha^{-j_l})}{\Lambda'(\alpha^{-j_l})} \cdot (\alpha^{-j_l})^{b-1}
    \]
    donde $\Lambda'(x)$ representa la derivada formal del polinomio localizador.
\end{itemize}
